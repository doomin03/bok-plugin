...
- `comparison-review.csv`: 차트별 유사도·상태·분석·검토 의견·적용 시각. 최초 생성 시 점수가 자동 기입된다.

## 오너의 버튼 기반 리뷰 저장

`comparison.html`에서 각 차트의 검토 의견에 “기존 차트 1개에서 2개로 증가” 같은 내용을 입력하고 **적용·저장**을 누른다. 해당 항목의 리뷰와 현재 계산된 유사도가 CSV에 함께 기록된다. 다른 항목의 미적용 입력은 저장에 포함하지 않는다. 여러 줄·쉼표·따옴표·한글을 보존하며 Excel용 UTF-8 BOM을 포함한다. 미측정은 빈 점수/pending이며 0점으로 쓰지 않는다.

Chrome/Edge 등 파일 접근 지원 환경에서는 **CSV 저장·연결**로 저장 파일을 한 번 선택하면 이후 적용 버튼이 같은 CSV를 갱신한다. 저장 중 실패나 외부 파일 변경이 감지되면 오류를 알리고 기존 적용 상태를 유지한다. 지원하지 않는 환경에서는 적용할 때 CSV를 다운로드한다. 다운로드 저장 위치는 브라우저 설정을 따른다.

다시 HTML을 열면 같은 브라우저에 적용된 리뷰를 임시 저장소에서 복원한다. 디스크의 CSV를 기준으로 복원하거나 다른 PC/브라우저에서 열 때는 **CSV 불러오기**로 저장한 파일을 선택한다. 지원 환경에서는 이 파일이 저장 대상으로도 연결된다. 브라우저는 허락 없이 기존 로컬 CSV를 자동 덮어쓸 수 없으므로, 재접속 후 파일 연결은 다시 선택한다. 브라우저 임시 저장은 CSV 백업을 대체하지 않는다.

CSV의 항목 ID·증거 해시·측정 점수를 검사해 다른 작업/변경된 이미지의 리뷰를 섞지 않는다. 리뷰 수정은 CSV에 저장되며 기존 review.md, owner-review.md, metrics.json을 자동 변경하거나 오너 승인을 기록하지 않는다. 에이전트는 후속 리뷰 시 최신 CSV도 읽고 실제 오너 결정과 구분한다.
